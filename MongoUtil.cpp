...
MongoClient::MongoClient(const char *host, int port) {
    if (!isIPAddress(host))
...
vector<string> MongoClient::database_names() {
    char **dbnames;
    unsigned i;
    bson_error_t *err = NULL;
    dbnames = mongoc_client_get_database_names(conn, err);
    db_names.clear(); /// get clear before push database names
    if (err == NULL)
    {
        for (i = 0; dbnames[i]; i++)
        {
            // cout<<dbnames[i]<<endl;
            db_names.push_back(string(dbnames[i]));
        }
        bson_strfreev(dbnames);
    }
    vector<string>(db_names).swap(db_names);
    return db_names;
}
